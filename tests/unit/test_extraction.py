...
def test_normalizer():
    ...
    ), "The number of normalized elements should be at most the specified value."
    ...
    for bucket in random_concepts_buckets:
        ...
        original_concepts_str = "; ".join([f'"{concept}"' for concept in bucket])
        normalized_concepts_str = "; ".join(
            [f'"{concept}"' for concept in normalized_concept]
        )
        ...
        assert proposition_holds(
            combined_text
            + "PROPOSITION: Most of the normalized concepts are semantically related to one or more of the original concepts."
        )
